...
const SOURCE_URL = process.env.TOKYO_SOURCE_URL || "https://www.tokyofes.info/";
const OUTPUT_PATH = path.resolve(__dirname, "../data/crawlTokyo.json");
...
function expandDateRange(dateText, headingMonth) {
  ...
  const matches = [...normalized.matchAll(/(?:(\d{4})年)?(?:(\d{1,2})月)?(\d{1,2})日/g)];
  ...
  const startMonth = normalized.startsWith("開催中")
    ? headingMonth
    : Number(startMatch[2]);
  const startDay = normalized.startsWith("開催中") ? 1 : Number(startMatch[3]);
...
function parseSchedule(html) {
  ...
  for (const monthMatch of html.matchAll(monthPattern)) {
    ...
    if (!heading.includes(String(YEAR)) || !headingMonth) continue;
...
async function crawlTokyoEvents() {
  ...
  if (events.length === 0) {
    throw new Error("No Tokyo events found. The source markup may have changed.");
  }

  fs.writeFileSync(OUTPUT_PATH, `${JSON.stringify(events, null, 2)}\n`);
  console.log(`Saved ${events.length} Tokyo events to ${OUTPUT_PATH}`);
  console.log(`Date range: ${events[0].date} - ${events.at(-1).date}`);
}

crawlTokyoEvents().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
